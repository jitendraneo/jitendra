...
class AW_Blogs_Model_Post extends Mage_Core_Model_Abstract
{
    const NOROUTE_PAGE_ID = 'no-route';

    protected function _construct()
    {
        $this->_init('blogs/post');
    }
    ...
    public function getShortContent()
    {
        ...
        if (Mage::getStoreConfig(AW_Blogs_Helper_Config::XML_BLOGS_PARSE_CMS)) {
    ...
    {
        $route = Mage::getStoreConfig('blogs/blogs/route');
        if ($route == "") {
            $route = "blogs";
        }
        $route = Mage::getUrl($route);

        $cats = Mage::getModel('blogs/cat')->getCollection()
