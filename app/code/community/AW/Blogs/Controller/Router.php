...
class AW_Blogs_Controller_Router extends Mage_Core_Controller_Varien_Router_Abstract
{
    public function initControllerRouters($observer)
    {
        $front = $observer->getEvent()->getFront();
        $blogs = new AW_Blogs_Controller_Router();
        $front->addRouter('blogs', $blogs);
    }
    ...
    public function match(Zend_Controller_Request_Http $request)
    {
        ...
        if (substr($identifier, 0, strlen('tag/')) == 'tag/') {
            ...
            if (!Mage::helper('blogs')->isTagAvailable($identifier)) {
                return false;
            }
            ...
            $cat = Mage::getSingleton('blogs/cat');
