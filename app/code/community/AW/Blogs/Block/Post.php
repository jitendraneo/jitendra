...
class AW_Blogs_Block_Post extends AW_Blogs_Block_Abstract
{
    ...
    public function getPost()
    {
        if (!$this->hasData('post')) {
            if ($this->getPostId()) {
                $post = Mage::getModel('blogs/post')->load($this->getPostId());
            } else {
                $post = Mage::getSingleton('blogs/post');
            }
            $category = Mage::getSingleton('blogs/cat')->load(
            ...
            if ($category->getIdentifier()) {
                ...
                $post->setAddress($this->getBlogsUrl($post->getIdentifier()));
            }
    ...
    {
        return Mage::getStoreConfig('blogs/comments/enabled');
    }

    public function getLoginRequired()
    {
        return Mage::getStoreConfig('blogs/comments/login');
    }
    ...
    {
        Mage::helper('blogs/toolbar')->create(
            $this,
            array(
    ...
    protected function _prepareCrumbs()
    {
        ...
        if ($breadcrumbs) {
            ...
            $breadcrumbs->addCrumb(
                'blogs_page', array('label' => htmlspecialchars_decode($this->getPost()->getTitle()))
            );
        }
    ...
    protected function getCategory()
    {
        if (!$this->hasData('postCategory')) {
            $this->setData(
                'postCategory', Mage::getSingleton('blogs/cat')->load($this->getRequest()->getParam('cat'), "identifier")
            );
        }
    ...
    {
        $blogsPostModelFromSession = Mage::getSingleton('customer/session')->getBlogsPostModel();
        if ($blogsPostModelFromSession) {
            return $blogsPostModelFromSession->getComment();
        }
    ...
    public function getCommentEmail()
    {
        $blogsPostModelFromSession = Mage::getSingleton('customer/session')->getBlogsPostModel();
        if ($blogsPostModelFromSession) {
            return $blogsPostModelFromSession->getEmail();
        }
    ...
    public function getCommentName()
    {
        ...
        $name = null;
        if ($blogsPostModelFromSession) {
            $name = $blogsPostModelFromSession->getUser();
        }
