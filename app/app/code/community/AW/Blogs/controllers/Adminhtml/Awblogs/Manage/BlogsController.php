...
class AW_Blogs_Adminhtml_Awblogs_Manage_BlogsController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('admin/blogs/posts');
    }
    ...
    public function editAction()
    {
        ...
        $model = Mage::getModel('blogs/blogs')->load($id);
        ...
        if ($model->getId() || $id == 0) {
            ...
            $this
                ->_addContent($this->getLayout()->createBlock('blogs/manage_blogs_edit'))
                ->_addLeft($this->getLayout()->createBlock('blogs/manage_blogs_edit_tabs'))
            ;
    ...
    public function duplicateAction()
    {
        ...
        while (Mage::getModel('blogs/post')->loadByIdentifier($newIdentifier)->getData()) {
            $newIdentifier = $oldIdentifier . ++$i;
        }
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            $model = Mage::getModel('blogs/post');
            if (isset($data['tags'])) {
                ...
                foreach ($tags as $key => $tag) {
                    $tags[$key] = Mage::helper('blogs')->convertSlashes($tag, 'forward');
                }
            ...
            try {
                ...
                foreach ($affectedTags as $tag) {
                    foreach ($stores as $store) {
                        if (trim($tag)) {
                            Mage::getModel('blogs/tag')->loadByName($tag, $store)->refreshCount();
                        }
                    }
                }

                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('blogs')->__('Post was successfully saved')
                );
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('blogs')->__('Unable to find post to save'));
        $this->_redirect('*/*/');
    }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($blogsIds)) {
            ...
            try {
                foreach ($blogsIds as $postId) {
                    $this->_postDelete($postId);
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d record(s) were successfully deleted', count($blogsIds)
                    )
                );
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
    ...
    {
        $model = Mage::getModel('blogs/blogs')->load($postId);
        ...
        foreach ($_tags as $tag) {
            foreach ($_stores as $store) {
                if (trim($tag)) {
                    Mage::getModel('blogs/tag')->loadByName($tag, $store->getId())->refreshCount();
                }
            }
        }
    }
        ...
        if (!is_array($blogsIds)) {
            ...
            try {
                ...
                foreach ($blogsIds as $blogsId) {
                    $blogs = Mage::getModel('blogs/blogs')
                        ->load($blogsId)
                ...
                $this->_getSession()->addSuccess(
                    $this->__('Total of %d record(s) were successfully updated', count($blogsIds))
                );
            } catch (Exception $e) {
