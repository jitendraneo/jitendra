...
try {
    $installer->run("
        CREATE TABLE IF NOT EXISTS {$this->getTable('blogs/blogs')} (
        ...
        CREATE TABLE IF NOT EXISTS {$this->getTable('blogs/comment')} (
        ...
        CREATE TABLE IF NOT EXISTS {$this->getTable('blogs/cat')} (
        ...
        INSERT INTO {$this->getTable('blogs/cat')} (`cat_id`, `title`, `identifier`) VALUES (NULL, 'News', 'news');

        CREATE TABLE IF NOT EXISTS {$this->getTable('blogs/store')} (
            `post_id` smallint(6) unsigned,
            `store_id` smallint(6) unsigned
        ) ENGINE = InnoDB DEFAULT CHARSET = utf8;

        CREATE TABLE IF NOT EXISTS {$this->getTable('blogs/cat_store')} (
            `cat_id` smallint(6) unsigned,
            `store_id` smallint(6) unsigned
        ) ENGINE = InnoDB DEFAULT CHARSET = utf8;

        CREATE TABLE IF NOT EXISTS {$this->getTable('blogs/post_cat')} (
            `cat_id` smallint(6) unsigned,
            `post_id` smallint(6) unsigned
        ) ENGINE = InnoDB DEFAULT CHARSET = utf8;
