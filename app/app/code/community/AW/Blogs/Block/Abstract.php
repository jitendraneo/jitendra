...
abstract class AW_Blogs_Block_Abstract extends Mage_Core_Block_Template
{
    ...
    protected static $_catUriParam = AW_Blogs_Helper_Data::CATEGORY_URI_PARAM;

    protected static $_postUriParam = AW_Blogs_Helper_Data::POST_URI_PARAM;

    protected static $_tagUriParam = AW_Blogs_Helper_Data::TAG_URI_PARAM;

    protected function _construct()
    {
        if (!self::$_helper) {
            self::$_helper = Mage::helper('blogs');
        }
    }
    ...
    protected function _processCollection($collection, $category = false)
    {
        ...
        foreach ($collection as $item) {
            ...
            if ($shortContent) {
                ...
                $strManager = new AW_Blogs_Helper_Substring(
                    array(
    ...
    {
        if (self::$_helper->conf(AW_Blogs_Helper_Data::XML_BOOKMARKS)) {
            return $this->setTemplate('aw_blogs/bookmark.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getTagsHtml($post)
    {
        if (trim($post->getTags())) {
            return $this->setTemplate('aw_blogs/line_tags.phtml')->setPost($post)->renderView();
        }
    }
    ...
    public function getBlogsUrl($route = null, $params = array())
    {
        $blogsRoute = self::$_helper->getRoute();
        if (is_array($route)) {
        ...
        foreach ($params as $key => $value) {
            ...
            $blogsRoute .= "{$key}/{$value}/";
        }

        return $this->getUrl($blogsRoute);
    }
    ...
    {
        $this->_helper('blogs/toolbar')->create(
            $this,
            array(
    ...
    protected function _prepareMetaData($meta)
    {
        if (is_object($meta)) {
            ...
            $openGraph = $head->getChild('aw_blogs_og');
            if ($openGraph) {
    ...
    {
        return '<a class="aw-blogs-read-more" href="' . $item->getAddress() . '">' . $this->__('Read More') . '</a>';
    }
    ...
    public function addBlogsLink($type)
    {
            ...
            if ($this->getParentBlock()) {
                if ($type == self::LINK_TYPE_HEADER) {
                    ...
                    $this->getParentBlock()->addLink(
                        $title, self::$_helper->getRoute(), $title, true, array(), 15, null, 'class="top-link-blogs"'
                    );
                }
            }
        }
    ...
    protected function _helper($type = 'blogs')
    {
    ...
    protected function _prepareRelatedCategories($collection)
    {
        ...
        $categories = Mage::getModel('blogs/post')->getCategoriesForPosts($posts);

        foreach ($categories as &$category) {
            $category['posts'] = explode(',', $category['posts']);
            $category['data'] = array(
                'title' => $category['title'],
                'url'   => $this->getBlogsUrl(null, array(self::$_catUriParam => $category['identifier']))
            );
        }
    ...
    public function isBlogsPage()
    {
        return Mage::app()->getRequest()->getModuleName() == AW_Blogs_Helper_Data::DEFAULT_ROOT;
    }
    ...
    protected function _prepareCollection()
    {
        if (!$this->getData('cached_collection')) {
            ...
            $sortDirection = Mage::helper('blogs')->defaultPostSort(Mage::app()->getStore()->getId());
            $collection = Mage::getModel('blogs/blogs')->getCollection()
                ->addPresentFilter()
                ->addEnableFilter(AW_Blogs_Model_Status::STATUS_ENABLED)
                ->addStoreFilter()
                ->joinComments()
