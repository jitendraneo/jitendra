...
class AW_Blogs_Model_Mysql4_Blogs extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {
        $this->_init('blogs/blogs', 'post_id');
    }
    ...
    {
        // Cleanup stats on blogs delete
        $adapter = $this->_getWriteAdapter();
        // 1. Delete blogs/store
        $adapter->delete($this->getTable('blogs/store'), 'post_id=' . $object->getId());
        // 2. Delete blogs/post_cat
        $adapter->delete($this->getTable('blogs/post_cat'), 'post_id=' . $object->getId());
        // 3. Delete blogs/post_comment
        $adapter->delete($this->getTable('blogs/comment'), 'post_id=' . $object->getId());
        // Update tags
        ...
        try {
            ...
            if (count($tags)) {
                foreach ($tags as $tag) {
                    Mage::getModel('blogs/tag')->loadByName($tag, null)->refreshCount();
                }
            }
        } catch (Exception $e) {
            Mage::logException($e);
